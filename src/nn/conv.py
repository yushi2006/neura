...
class Conv2d(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int, padding: Union[tuple, int] = 0, stride: Union[tuple, int] = 1, has_bias: bool = False):
        super().__init__()
        ...
        self.W = neura.Tensor(np.random.randn(*weight_shape).astype(np.float32) * scale, requires_grad=True)
        
        self.b = neura.Tensor(np.zeros((self.out_channels, 1, 1))) if self.has_bias else None
